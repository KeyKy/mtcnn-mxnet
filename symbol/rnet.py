...
def get_symbol(is_train=True):
    ...
    conv0 = conv(data=data, num_filter=28, kernel=(3, 3), stride=(1, 1), pad=(0, 0), name='0')
    pool0 = mx.sym.Pooling(data=conv0, kernel=(3, 3), stride=(2, 2), pad=(0, 0), pooling_convention='full', pool_type='max', name='pool_0')

    conv1 = conv(data=pool0, num_filter=48, kernel=(3, 3), stride=(1, 1), pad=(0, 0), name='1')
    pool1 = mx.sym.Pooling(data=conv1, kernel=(3, 3), stride=(2, 2), pad=(0, 0), pooling_convention='full', pool_type='max', name='pool_1')

    conv2 = conv(data=pool1, num_filter=64, kernel=(2, 2), stride=(1, 1), pad=(0, 0), name='2')

    fc = mx.sym.FullyConnected(data=conv2, num_hidden=128, name='fc')
